...
- `src/widget.ts` — Entry point. Fetches GPS location via Scriptable's `Location` API, converts temperatures from °C to °F, then builds and presents a `ListWidget`. Exports `{ main }` for the auto-updater pattern.
...
```
GPS (Location API)
  → Open-Meteo forecast API (hourly, °C)  [weather.ts]
    → celsiusToFahrenheit()               [weather.ts]
      → getGearAdvice() [outdoor days only]
        → ListWidget render
```

**Key unit convention:** The `Weather` interface in `weather.ts` stores temperatures in °C exactly as the API returns them. Conversion to °F happens in `main()` before any display or call to `getGearAdvice`. The `WeatherConditions` interface in `gearAdvice.ts` expects °F — never pass raw API values to it.
